...
For strict review, run the main skill's structured file review (`open-code-review-delegate`) on the current head and map findings with the `/git-review-pr` mapping. OCR Step 7 Fix stays off. Then review correctness and regression risk; automated tests and reproducible validation; APIs, schemas, generated artifacts, and data contracts; security; required CI; deployment and operations; and agreement between documentation and behavior. A substantive unverified risk blocks approval. Style-only preferences stay non-blocking unless repository policy requires them.
