...
15. Keep the PR/MR description current: summary, issue link, validation evidence with each brief Test case id and its result, known limitations, reviewer/assignee metadata when available, any pre-submit waivers, and the Proof record from `references/pre-submit.md`. Under `## Verification`, outside that Proof record, optional observation lines may be added: `Brief: followed | adjusted: <step ids> | dissent <comment id>` and `Executor-model: <id or unknown>`. A missing line does not fail the pre-submit gate.
