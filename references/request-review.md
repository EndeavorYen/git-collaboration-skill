...
Do not change code or merge. Do not send a duplicate request when a current-head request is already pending. The operator reply ends with one `next step:` line from the review handoff in `SKILL.md`. Do not print the Solo override block.
