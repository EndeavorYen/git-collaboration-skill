...
Run the skill's read-only PR/MR command preflight first, including the actor gate. Continue only when the PR/MR is not `owned` and not `self_authored_head`. If the authenticated user is the author or a current assignee, or any commit on the current PR/MR was authored or committed by that user, classify `REVIEW_NOT_AUTHORIZED` and do not post review feedback or approve. A named IID, URL, or list does not authorize self-review. Print Solo override for `/git-review-pr-force` when the user owns the PR/MR or has commits on it.
...
Otherwise obey **Forge budget**: one PR/MR snapshot, then review that head in the local checkout. Run the skill's structured file review with `open-code-review-delegate` on the current head, map findings with the skill's `/git-review-pr` mapping, then continue CI, evidence-class, remaining-gate, and verdict rules. OCR Step 7 Fix stays off. Keep the local workflow read-only except for forge review feedback, and post a visible approve/block verdict in the issue/PR language or the repo's documented language. The review write response is the read-back for head SHA, pipeline, discussion state, and approval/request-changes state. One confirm view only when that response omits the head SHA or approval state.
