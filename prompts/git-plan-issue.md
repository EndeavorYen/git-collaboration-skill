...
Do not post a duplicate brief. Do not implement or open a PR/MR. Do not post a brief while an unsettled product decision remains in Goal, Recommended change, Out of scope, or Acceptance criteria. A deferred or open assumption is still unsettled. Load `gentle-grill-me` and post nothing until the user confirms the close log. After that confirmation, rewrite those four fields from settled log entries only. If any field is still deferred or open, post nothing. Scheduled runs and `/git-triage` do not load `gentle-grill-me`; recommend `/git-plan-issue`.
