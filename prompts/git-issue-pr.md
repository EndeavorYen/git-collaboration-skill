...
If you agree with Goal, Recommended change, Out of scope, and Acceptance criteria on the current brief, treat that note as the implementation contract, report which note id supplied it, and implement. Local details inside that shape stay on this path. A human decision in a later comment, a newer brief, or this invocation is final and becomes the implementation contract. If no current brief exists, or those four fields still contain an unsettled product decision, stop without editing code and do not load `gentle-grill-me`. Next command: `/git-plan-issue` with the exact issue URL.
